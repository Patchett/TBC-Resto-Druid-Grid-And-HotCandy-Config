...
local MINOR = tonumber(("$Revision: 81345 $"):match("%d+"))
...
do
	...
	function lib:NewBarGroup(name, orientation, length, thickness, frameName)
		...
		-- Only changed needed for WotLK.
		if list.button.SetTextFontObject then
...
function barListPrototype:UpdateOrientationLayout()
	...
	barListPrototype.super.SetWidth(self, vertical and thickness or length)
	barListPrototype.super.SetHeight(self, vertical and length or thickness)
...
do
	...
	function barListPrototype:SortBars()
		...
		for i = 1, #values do
			local realx, realy = xspacing, yspacing
			local realto = to
			if collapse then
				realto = from
				realx = xoff
...
do
	function barPrototype:UpdateOrientationLayout()
		...
		if o == lib.LEFT_TO_RIGHT then
			...
			t:SetPoint("BOTTOMRIGHT", self.timerLabel, "BOTTOMLEFT", 0, 0)
			t:SetJustifyH("LEFT")
			t:SetJustifyV("MIDDLE")
		...
		elseif o == lib.RIGHT_TO_LEFT then
			...
			t:ClearAllPoints()
			t:SetPoint("TOPRIGHT", self, "TOPRIGHT", -3, -3)
			t:SetPoint("BOTTOMLEFT", self.timerLabel, "BOTTOMRIGHT", 0, 0)
			t:SetJustifyH("RIGHT")
			t:SetJustifyV("MIDDLE")
...
function barPrototype:SetLength(length)
	self.length = length
	local vertical, thickness = self.orientation % 2 == 0, self.thickness
	barPrototype.super.SetWidth(self, vertical and thickness or length)
	barPrototype.super.SetHeight(self, vertical and length or thickness)
end
...
function barPrototype:SetThickness(thickness)
	self.thickness = thickness
	local vertical, length = self.orientation % 2 == 0, self.length
	barPrototype.super.SetWidth(self, vertical and thickness or length)
	barPrototype.super.SetHeight(self, vertical and length or thickness)
